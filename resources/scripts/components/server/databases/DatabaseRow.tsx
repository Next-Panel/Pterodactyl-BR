...
export default ({ database, className }: Props) => {
    ...
    const schema = object().shape({
        confirm: string()
            .required('O nome do banco de dados deve ser informado.')
            .oneOf([database.name.split('_', 2)[1], database.name], 'O nome do banco de dados deve ser informado.'),
    });
        ...
        <>
            <Formik onSubmit={submit} initialValues={{ confirm: '' }} validationSchema={schema} isInitialValid={false}>
                {({ isSubmitting, isValid, resetForm }) => (
                    ...
                    >
                        ...
                        <h2 css={tw`text-2xl mb-6`}>Confirmar exclusão do banco de dados</h2>
                        <p css={tw`text-sm`}>
                            A exclusão de um banco de dados é uma ação permanente, não pode ser desfeita. Isso excluirá permanentemente o banco de dados <strong>{database.name}</strong> e removerá todos os dados associados.
                        </p>
                        <Form css={tw`m-0 mt-6`}>
                            <Field
                                ...
                                id={'confirm_name'}
                                name={'confirm'}
                                label={'Confirmar Nome do Banco de Dados'}
            ...
            <GreyRowBox $hoverable={false} className={className} css={tw`mb-2`}>
                ...
                <div css={tw`ml-8 text-center hidden md:block`}>
                    ...
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>Endereço</p>
                </div>
                <div css={tw`ml-8 text-center hidden md:block`}>
