    ...
    <div class="col-xs-12">
        <div class="nav-tabs-custom nav-tabs-floating">
            <ul class="nav nav-tabs">
                ...
                <li><a href="{{ route('admin.nodes.view.allocation', $node->id) }}">Alocação</a></li>
    ...
    <div class="col-sm-8">
        <div class="box box-primary">
            ...
            <div class="box-footer">
                <p class="no-margin">Este arquivo deve ser colocado no diretório raiz do seu daemon (geralmente <code>/etc/pterodactyl</code>) em um arquivo chamado <code>config.yml</code>.</p>
            </div>
        </div>
    </div>
    <div class="col-sm-4">
        <div class="box box-success">
            ...
            <div class="box-body">
                <p class="text-muted small">
                    ...
                    as asas no servidor de destino com um único comando.
                </p>
            </div>
            <div class="box-footer">
    ...
    $('#configTokenBtn').on('click', function (event) {
        $.ajax({
            ...
            swal({
                type: 'success',
                title: 'Token criado.',
                text: '<p>Para autoconfigurar seu Node, execute o seguinte comando:<br /><small><pre>cd /etc/pterodactyl && sudo wings configure --panel-url {{ config('app.url') }} --token ' + data.token + ' --node ' + data.node + '{{ config('app.debug') ? ' --allow-insecure' : '' }}</pre></small></p>',
                html: true
            })
        }).fail(function () {
            swal({
                title: 'Erro',
                text: 'Ocorreu um erro ao criar seu token.',
                type: 'error'
            });
        });
    });
