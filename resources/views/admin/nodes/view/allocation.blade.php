...
@section('content-header')
    <h1>{{ $node->name }}<small>Controle as alocações disponíveis para servidores neste Node.</small></h1>
    <ol class="breadcrumb">
    ...
    <div class="col-xs-12">
        <div class="nav-tabs-custom nav-tabs-floating">
            <ul class="nav nav-tabs">
                ...
                <li><a href="{{ route('admin.nodes.view.settings', $node->id) }}">Configurações</a></li>
                <li><a href="{{ route('admin.nodes.view.configuration', $node->id) }}">Configuração</a></li>
                <li class="active"><a href="{{ route('admin.nodes.view.allocation', $node->id) }}">Alocação</a></li>
            ...
            <div class="box-body table-responsive no-padding" style="overflow-x: visible">
                <table class="table table-hover" style="margin-bottom:0;">
                    <tr>
                        ...
                        <th>Endereço de IP <i class="fa fa-fw fa-minus-square" style="font-weight:normal;color:#d9534f;cursor:pointer;" data-toggle="modal" data-target="#allocationModal"></i></th>
                        <th>Alias de IP</th>
                        <th>Porta</th>
                        <th>Atribuído a</th>
                        <th>
                            <div class="btn-group hidden-xs">
                                <button type="button" id="mass_actions" class="btn btn-sm btn-default dropdown-toggle disabled"
                                        data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Ações em Massa <span class="caret"></span>
                                </button>
                                <ul class="dropdown-menu dropdown-massactions">
                                    <li><a href="#" id="selective-deletion" data-action="selective-deletion">Excluir <i class="fa fa-fw fa-trash-o"></i></a></li>
                                </ul>
                            </div>
                        </th>
                    </tr>
                    @foreach($node->allocations as $allocation)
                        <tr>
                            ...
                            <td class="col-sm-3 middle">
                                <input class="form-control input-sm" type="text" value="{{ $allocation->ip_alias }}" data-action="set-alias" data-id="{{ $allocation->id }}" placeholder="nenhum" />
            ...
            <div class="box box-success">
                ...
                <div class="box-body">
                    <div class="form-group">
                        <label for="pAllocationIP" class="control-label">Endereço de IP</label>
                        <div>
                            ...
                            <p class="text-muted small">Insira um endereço de IP para atribuir portas aqui.</p>
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="pAllocationIP" class="control-label">Alias de IP</label>
                        <div>
                            <input type="text" id="pAllocationAlias" class="form-control" name="allocation_alias" placeholder="alias" />
                            <p class="text-muted small">Se você gostaria de atribuir um alias padrão a essas alocações, insira-o aqui.</p>
                        </div>
                    </div>
                    <div class="form-group">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Excluir Alocações para Bloco de IP</h4>
            </div>
...
@section('footer-scripts')
    ...
    $('button[data-action="deallocate"]').click(function (event) {
        ...
        swal({
            title: '',
            text: 'Tem certeza de que deseja excluir esta alocação?',
            type: 'warning',
            ...
            $.ajax({
                ...
                swal({ type: 'success', title: 'Porta Excluída!' });
            }).fail(function (jqXHR) {
                console.error(jqXHR);
                swal({
                    title: 'Opa!',
    ...
    function deleteSelected() {
        ...
        if (selectedItems.length !== 0) {
            ...
            if (selectedItems.length > 5) {
                formattedItems += ', e ' + (selectedItems.length - 5) + ' outro(s)';
            }
            ...
            swal({
                ...
                $.ajax({
                    ...
                    swal({
                        ...
                        title: 'Opa!',
                        html: true,
                        text: 'Ocorreu um erro ao tentar excluir estas alocações. Por favor, tente novamente.',
                    });
                });
            });
        } else {
            swal({
                type: 'warning',
                title: '',
                text: 'Por favor, selecione a(s) alocação(ões) a serem excluídas.',
            });
        }
    }
    </script>
