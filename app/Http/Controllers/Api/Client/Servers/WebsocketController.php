...
class WebsocketController extends ClientApiController
{
    ...
    public function __invoke(ClientApiRequest $request, Server $server): JsonResponse
    {
        ...
        if ($user->cannot(Permission::ACTION_WEBSOCKET_CONNECT, $server)) {
            throw new HttpForbiddenException('Você não tem permissão para conectar-se ao websocket deste servidor.');
        }
        ...
        if (!is_null($server->transfer)) {
            ...
            if (!in_array('admin.websocket.transfer', $permissions)) {
                throw new HttpForbiddenException('Você não tem permissão para visualizar os logs de transferência do servidor.');
            }
