...
class ScheduleTaskController extends ClientApiController
{
    ...
    public function store(StoreTaskRequest $request, Server $server, Schedule $schedule): array
    {
        ...
        if ($schedule->tasks()->count() >= $limit) {
            throw new ServiceLimitExceededException("Os cronogramas não podem ter mais do que $limit de tarefas associadas a eles. A criação desta tarefa colocaria este cronograma acima do limite.");
        }
    ...
    public function delete(ClientApiRequest $request, Server $server, Schedule $schedule, Task $task): JsonResponse
    {
        ...
        if (!$request->user()->can(Permission::ACTION_SCHEDULE_UPDATE, $server)) {
            throw new HttpForbiddenException('Você não tem permissão para realizar esta ação.');
        }
