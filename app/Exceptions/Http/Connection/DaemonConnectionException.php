...
class DaemonConnectionException extends DisplayException
{
    ...
    public function __construct(GuzzleException $previous, bool $useStatusCode = true)
    {
        ...
        if (is_null($response)) {
            $message = 'Não foi possível estabelecer uma conexão com a máquina que executa este servidor. Por favor, tente novamente.';
        } else {
            $message = sprintf('Ocorreu um erro durante a comunicação com a máquina que executa este servidor. Este erro foi registrado, por favor, tente novamente. (código: %s) (id_requisição: %s)', $response->getStatusCode(), $this->requestId ?? '<nil>');
        }
        ...
        if ($this->statusCode < 500 && !is_null($response)) {
            ...
            $message = sprintf('Ocorreu um erro no host remoto: %s. (id da requisição: %s)', $body['error'] ?? $message, $this->requestId ?? '<nil>');
        }
