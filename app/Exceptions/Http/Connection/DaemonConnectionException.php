...
class DaemonConnectionException extends DisplayException
{
    ...
    public function __construct(GuzzleException $previous, bool $useStatusCode = true)
    {
        ...
        if (is_null($response)) {
            $message = 'Não foi possível estabelecer uma conexão com a máquina rodando este servidor. Por favor, tente novamente.';
        } else {
            $message = sprintf('Houve um erro durante a comunicação com a máquina rodando este servidor. Este erro foi registrado, por favor tente novamente. (code: %s) (request_id: %s)', $response->getStatusCode(), $this->requestId ?? '<nil>');
        }
        ...
        if ($this->statusCode < 500 && !is_null($response)) {
            ...
            $message = sprintf('Ocorreu um erro no host remoto: %s. (request id: %s)', $body['error'] ?? $message, $this->requestId ?? '<nil>');
        }
