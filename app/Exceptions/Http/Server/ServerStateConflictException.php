...
class ServerStateConflictException extends ConflictHttpException
{
    ...
    public function __construct(Server $server, ?\Throwable $previous = null)
    {
        ...
        if ($server->isSuspended()) {
            ...
            $message = 'Este servidor ainda não concluiu seu processo de instalação, por favor, tente novamente mais tarde.';
        } elseif ($server->status === Server::STATUS_RESTORING_BACKUP) {
            $message = 'Este servidor está sendo restaurado de um backup, por favor, tente novamente mais tarde.';
        } elseif (!is_null($server->transfer)) {
            $message = 'Este servidor está sendo transferido para uma nova máquina, por favor, tente novamente mais tarde.';
        }
