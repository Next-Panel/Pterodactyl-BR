...
class DatabaseSettingsCommand extends Command
{
    ...
    {
        $this->output->note('É altamente recomendável não usar "localhost" como host do banco de dados, pois vimos problemas frequentes de conexão de soquete. Se você deseja usar uma conexão local, deve usar "127.0.0.1".');
        ...
        $this->output->note('Usar a conta "root" para conexões MySQL não é apenas altamente desaprovado, mas também não é permitido por este aplicativo. Você precisará ter criado um usuário MySQL para este software.');
        ...
        if (!empty(config('database.connections.mysql.password')) && $this->input->isInteractive()) {
            ...
            $askForMySQLPassword = $this->confirm('Parece que você já tem uma senha de conexão MySQL definida, gostaria de alterá-la?');
        }

        if ($askForMySQLPassword) {
            $this->variables['DB_PASSWORD'] = $this->option('password') ?? $this->secret('Senha do Banco de Dados');
        }

        try {
            $this->testMySQLConnection();
        } catch (\PDOException $exception) {
            $this->output->error(sprintf('Não foi possível conectar ao servidor MySQL usando as credenciais fornecidas. O erro retornado foi "%s".', $exception->getMessage()));
